...
class Matrix {
    ...
    multiplyGpu(other) {
     ...
     //use multiplyMatrix function from gpu.js
     //set its output to [this.rows,other.cols]

        //set multiplyMatrix.setOutput
        multiplyMatrix.setOutput([this.cols,other.rows]);

        let result = multiplyMatrix(this.data,other.data);
        result = result.slice(0,this.rows).map((row)=>row.slice(0,other.cols));
